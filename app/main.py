...
import requests
from .models import Kelompok_Tani, User, Desa, Kecamatan, Kabupaten, Provinsi, Peta_Desa, Data_Spasial
from . import db
import re
...
def gis():
    ...
    mycursor.execute(
        "SELECT ST_AsText(SHAPE) AS SHAPE FROM data__spasial;")

    all_data = mycursor.fetchall()
    # all_data = Data_Spasial.query.all()
    ...
    for x in all_data:
        ...
        # konversi longlat ke latlong
        kordinat = s
        kordinat = kordinat[9:]
        kordinat = kordinat[:-2]
        kordinat = kordinat.replace('(', '[')
        kordinat = kordinat.replace(')', ']')
        text = ""
        neww = kordinat.split(',')
        for x in neww:
            neww2 = x.split(' ')
            print(neww2[1]+" "+neww2[0])
            text = text + "(" + neww2[1] + " " + neww2[0] + "),"
        s = "POLYGON("+text+")"
